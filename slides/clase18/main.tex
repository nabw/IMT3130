...
\begin{frame}{Algebra of Differentials: Nanson's Formula}
    \begin{itemize}
        ...
        \item Jacobian $J = \det \ten F$ dictates the transformation of volume elements: $dv = J dV$.
...
\begin{frame}{Velocities and the Material Derivative}
    \begin{itemize}
        \item \textbf{Material/Lagrangian/Reference quantities}: $\eta(\vec X, t)$
        \item \textbf{Spatial/Eulerian/Deformed quantities}: $\eta(\vec x, t)$
    \end{itemize}

    \pause Velocities:
    \begin{itemize}
        \item \textbf{Material Velocity:} $\bm{V}(\bm{X}, t) = \frac{\partial \bm{\varphi}(\bm{X}, t)}{\partial t}$.
        \item \textbf{Spatial Velocity:} $\bm{v}(\bm{x}, t) = \bm{V}(\bm{\varphi}_t^{-1}(\bm{x}),t)$.
    \end{itemize}
