...
\begin{frame}{Step 1: Induced Operators and the Riesz Map}
    ...
    By the Riesz Representation Theorem, there exists an isometric isomorphism (the Riesz map) $R: H \to H'$.
    ...
    $$ Au = f \text{ in } H' \iff (R^{-1} \circ A)u = R^{-1}f \text{ in } H $$
\end{frame}

\begin{frame}{Step 2: Fixed Point Formulation}
    To prove existence and uniqueness, we construct a fixed-point mapping.

    Let $F = R^{-1}f \in H$. For a parameter $\rho > 0$, define the map $T: H \to H$ as:
    $$ T(u) = u - \rho R^{-1} \circ (Au - f) = u - \rho(R^{-1}Au - F) $$

    Clearly, $u$ is a fixed point of $T$ (i.e., $T(u) = u$) if and only if $Au = f$. We seek to find a $\rho$ such that $T$ is a strict contraction on $H$.
\end{frame}
...
\begin{frame}{Step 4: Stability Estimate}
    Finally, we derive the \textit{a priori} estimate directly from the properties of the bilinear form.

    By ellipticity, we have:
    $$ \alpha\|u\|_H^2 \le a(u, u) $$

    Since $u$ is the solution, $a(u,u) = f(u)$. By the definition of the dual norm:
