...
\begin{frame}{Three Fundamental Schemes}
    \begin{itemize}
        \item \textbf{Left-sided rule (Explicit):} Evaluates the operator at the \textbf{known} time $t_n$.
        \[
            \int_{t_n}^{t_{n+1}} A_h x_h(s) ds \approx \Delta t A_h x_h^n
        \]
        \item \textbf{Right-sided rule (Implicit):} Evaluates the operator at the \alert{unknown} time $t_{n+1}$.
        \[
...
\begin{frame}{Moving to the Frequency Domain}
    Energy methods for stability analysis can become intractable $\to$  \textbf{Von Neumann stability analysis}
    
    Relies on Fourier transform + translation-invariant finite differences.
    
    We consider a basis function for a given wave number $\xi$ evaluated on the grid $x_j = jh$:
    \[
        W_j = e^{i\xi(jh)} (\approx e^{i\xi x})
    \]
    This ansatz decouples degrees of freedom:
        
    \vspace{1cm}
      Find an amplification factor $g(\xi)$ such that $\hat{u}^{n+1} = g(\xi)\hat{u}^n$.
\end{frame}
...
\begin{frame}{Extracting the Amplification Factor}
    ...
    \begin{align*}
        g(\xi) &= 1 + \frac{\Delta t}{h^2}(e^{-i\xi h} + e^{i\xi h} - 2)  \\
               &= 1 + \frac{\Delta t}{h^2}(2\cos(\xi h) - 2) \quad \text{( Euler's formula)}  \\
