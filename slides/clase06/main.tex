...
\begin{frame}{Time Stability Definition}
    We seek stability estimates in time. Consider a general forward scheme written as $u^{n+1} = g(u^n)$.
    
    A condition such as $\|u^{n+1}\| \le c\|u^n\|$ for $c \le 1$ implies recursively that:
    \[
        \|u_h^n\|_{\infty,h} \le \|u_h^0\|_{\infty,h} 
    \]
    This uniform boundedness over time is precisely a stability result as in the \textbf{Lax Equivalence Theorem}.
\end{frame}
