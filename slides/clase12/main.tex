...
\begin{frame}{The 1D Mesh and $\P_1$ Elements}
   ...
   \pause The canonical basis for this space consists of \emph{hat functions} $\phi_i$:
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                ...
                \item $\phi_i'(x) = 1/h 1_{I_{i-1}} - 1/h 1_{I_i}$
            \end{itemize}
        \end{column}
    \end{columns}
            \begin{center}
...
$$ u_h(x_j) = \sum_i u_i \phi(x_j) = u_j \quad\Rightarrow\quad \text{Nodal values} $$
    ...
    \vspace{0.3cm}
    \pause \textbf{2. The Interpolation Operator:}
...
\begin{frame}{From Weak Form to Algebraic Equations}
    ...
    \vspace{0.3cm}
    \pause\textbf{Step 1: Test against basis functions.}
    ...
    \vspace{0.3cm}
    \pause\textbf{Step 2: Expand the trial function.}
...
\begin{frame}{The Stiffness Matrix and Load Vector}
    ...
    \begin{itemize}
        \item $U = (U_1, \dots, U_N) \in \R^N$ unknown nodal values or \textbf{DoFs} (Degrees of Freedom).
...
\begin{frame}{ADR cont'd}
    ...
    \begin{itemize}
        ...
        \item $K_{i, i\pm 1} = c \int_{x_i}^{x_{i+1}} \phi_{i+1}' \phi_i dx = c \int_{x_i}^{x_{i+1}} (\frac{\pm 1}{h})(\frac{x_{i+1}-x}{h}) dx = \frac{\pm c}{h^2} \left[x_{i+1}x - \frac{x^2}{2}\right]_{x_i}^{x_{i+1}} = \frac{c}{2}$.
    \end{itemize}

    \vspace{0.2cm}
    \textbf{3. Reaction Term:}
    \begin{itemize}
...
\begin{frame}[fragile]\frametitle{ADR: Code}
    \idea{Console}

    \vspace{0.5cm}
    Main tool to preserve sparsity: \code{scipy.sparse.diags}
    \vspace{0.5cm}
 ...
 K_diff = (nu / h) * diags([-off, 2*main, -off], [-1, 0, 1])
    \end{verbatim}
    \hrulefill
