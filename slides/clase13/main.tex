...
\begin{frame}{Mapping and Quadrature}
    ...
    Let $J = \frac{\partial x}{\partial \xi}$ be the Jacobian of the transformation. To compute integrals in FE: \pause
    \begin{enumerate}[<+->]
    ...
    $$ \nabla_x \phi_a^e = \frac{\partial \phi_a^e}{\partial x_i} = \frac{\partial \phi_a^e}{\partial \xi_j}\frac{\partial \xi_j}{\partial x_i} =  J^{-T}(\bm{\xi}) \nabla_\xi \hat{\phi}_a(\bm{\xi}) $$
        \item The  integrals $\int_{\hat{K}} (\dots) d\hat{\Omega}$ are evaluated using numerical quadrature  on $\hat{K}$.
            $$ \int_{\hat{K}} \psi(x(\xi)) | \det J(\xi) | d\hat{\Omega} \approx \sum_q \psi(x(\xi_q)) | \det J(\xi_q) | w_q $$
    \end{enumerate}
\end{frame}
...
\begin{frame}{The FE Assembly Challenge}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                \item Element \phantom{II}I: $1\quad 5\quad 4$
...
\begin{frame}{Connectivity and Equation Numbering}
    \begin{footnotesize}
    Assembly maps local element contributions $k^e_{ab}$ and $f^e_a$ to the global matrices $K_{ij}$ and $F_i$. \pause
    
    \vspace{0.3cm}
    \pause\textbf{0. The geometry (nodes):} A matrix $x_i$ with the coordinates of the global node $i$.
    
    \pause\textbf{1. The Element Node Matrix (IEN):}
    $$ \text{IEN}[e, a] \in \mathbb{N} $$
    Maps local node $a \in \{1, \dots, n_{dof}\}$ of element $e \in \{1, \dots, N_{el}\}$ to its \textbf{global node index}.
    
    \vspace{0.3cm}
    \pause\textbf{2. The Destination Array (ID):}
    $$ \text{ID}[i] \in \mathbb{Z} $$
    Maps global node index $i$ to its \textbf{global DoF}\footnote{Dirichlet nodes are set to -1}. 

    \pause\textbf{3. The Location Matrix (LM):}
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            ...
            \begin{tikzpicture}[scale=2]
                ...
                \fill[color=red] (0,0) circle (1pt) node[below left] {1};
                \fill (1,0) circle (1pt) node[below right] {2};
                \fill (1,1) circle (1pt) node[above right] {3};
                \fill[color=red] (0,1) circle (1pt) node[above left] {4};
