...
\begin{frame}{The Interpolator Operator}
    ...
    $$ \Pi_h u(x) = \sum_{i=0}^N \sigma_i (u) \varphi(x) = \sum_{i=0}^N u(x_i) \varphi(x) $$
...
\begin{frame}{Results obtained}
    \begin{itemize}
        \item Hat functions span $V_h$
        \item The Lagrange interpolator $\Pi_h u = \sum_i u(x_i) \varphi_i$ satisfies:
                \begin{itemize}
                    \item It is bounded, $\| \Pi_h \| \leq C$
                    \item It is convergent in $H^1$, $\| u - \Pi_h u \|_1 \leq C_1 h \| u \|_2$
                    \item It is convergent in $L^2$, $\| u - \Pi_h u \|_0 \leq C_0 h^2 \| u \|_2$
                \end{itemize}
    \end{itemize}
    In particular: 
    $$ \| u - u_h \|_1 \leq \frac{C}{\alpha} \inf_{z_h\in V_h}\|u - z_h\| \leq \frac{C C_1}{\alpha} h \| u \|_2 $$

\end{frame}

\begin{frame}{Aubin-Nitsche Lemma}
    Consider $V=H_0^1(\Omega)$, $\Omega=(a,b)$, $u$ and $u_h$ solutions of
    $$ (\nabla u, \nabla v) = \langle f, v\rangle \quad\forall v\in V \qquad\qquad (\nabla u_h, \nabla v_h) = \langle f, v_h\rangle \quad\forall v_h \in V_h$$
    Define the error $e_h = u-u_h$, and the solution $z$ of dual problem
    $$ (\nabla z, \nabla v) = (e_h, v) \quad \forall v \in V $$
    Then, elliptic regularity yields $\|z\|_2 \leq C \| e_h \|_0$, so
    \begin{align*}
        \| e_h \|_0^2 &= (e_h, e_h) \\
                      &= (\nabla z, \nabla e_h) \tag{Def of dual problem}\\
                      &= (\nabla e_h, \nabla z - \Pi_h z) \tag{Galerkin Orth.} \\
                      &\leq C h\|z\|_2 \|e_h \|_1  \tag{Conv. of $z$}\\
                      &\leq C h^2 \| u \|_2 \|e_h \|_0 \tag{Elliptic reg.}
    \end{align*}
    Thus, 
