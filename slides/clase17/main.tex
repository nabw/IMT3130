...
\title{Topics beyond this course}
...
\begin{frame}{The Limits of Lax-Milgram}
    Throughout this course, we have relied heavily on the \textbf{Lax-Milgram lemma} and ended with \textbf{inf-sup}.

    \vspace{0.3cm}
    Many  models break this. Some include:
    \begin{itemize}
        \item Loss of coercivity (Compact perturbations).
        \item Constraints and Lagrange multipliers (Saddle point problems).
        \item Nonlinear  laws  (Nonlinear analysis).
...
\begin{frame}{1. Compact Perturbations \& Fredholm Theory}
    \textbf{Motivation:} High-frequency wave propagation, $$-\Delta u - k^2 u = f$$, where  $k^2$ term destroys ellipticity.

    \vspace{0.2cm}
    \textbf{The Abstraction:}
    $T=A - K$, where $A: V \to V'$ is elliptic isomorphism and $K: V \to V'$ is \textbf{compact operator}.
    ...
    \begin{itemize}
        ...
        \item Well-posedness reduces to proving that $u=0$ is the \textit{only} solution to the homogeneous problem.
        \item FE convergence for $h\leq h_0$.
    \end{itemize}
\end{frame}
...
\begin{frame}{2. Saddle Point Problems}
    \begin{columns}
        \begin{column}{0.6\textwidth}
            \textbf{Motivation:} Mixed formulations. We must minimize an energy functional subject to a hard constraint (e.g., $\text{div } u = 0$).
            ...
            \textbf{Analytical Toolkit:}
            \begin{itemize}
                \item \textbf{LBB Theory:} $A$ bijective and $B$ surjective.
...
\begin{frame}{3. Nonlinear Problems}
    \textbf{Motivation:} Large solid deformations, non-Newtonian fluids ($p$-Laplacian), or  Navier-Stokes. The mapping $u \mapsto A(u)$ is no longer linear.
    ...
    \item \textbf{Minty-Browder Theorem:} Guarantees existence and uniqueness for operators that are \emph{monotone}, \emph{coercive}, and \emph{hemicontinuous}.
    \end{itemize}
\end{frame}
...
\begin{frame}{4. Time-Dependent Problems (Evolution PDEs)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Semi-group theory:} Show that $A$ induces a family of semi-groups (Hille-Yosida Theorem).
    \end{itemize}
\end{frame}

\section{Optimal Control}
\begin{frame}{5. Optimal Control (PDE-Constrained Optimization)}
    \textbf{Motivation:} Engineering design.

    \vspace{0.2cm}
    \textbf{The Abstraction:}
            $$
            \begin{aligned}
                &\min_{z\in Z} &&  J(v(z)) + \alpha \| z \|^2 \\
                &\text{s.t.}\quad && \mathcal L v = f(z)
            \end{aligned}
            $$
    ...
    \begin{tikzpicture}[node distance=2.5cm, auto, thick]
        ...
        \node[draw, rectangle, fill=red!10, rounded corners, right of=control, node distance=4cm, minimum height=1cm] (state) {State $v(z)$ (PDE)};
        \node[draw, rectangle, fill=green!10, rounded corners, right of=state, node distance=4cm, minimum height=1cm] (cost) {Cost $\tilde J(u,z)=J(v(z)) $};
        
        \draw[->] (control) -- node[above] {PDE} (state);
        \draw[->] (state) -- (cost);
    ...
    \textbf{Analytical Toolkit:}
    \begin{itemize}
        \item \textbf{Adjoint calculus and inifinite dimensional KKT.}
    \end{itemize}
\end{frame}
