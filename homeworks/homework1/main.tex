...
\begin{enumerate}
    ...
    \item\pts{2 extra} Enuncie y demuestre la regla de la cadena para derivadas de Fréchet, dada por $d(G\circ F)(u)[h] = dG(F(u))[dF(u)[h]$. Luego, úsela para encontrar la derivada de la composición $S\circ T$ de los operadores $S:L^\infty(\Omega)\to L^\infty(\Omega)$ y $T:L^\infty(\Omega)\to L^\infty(\Omega)$ dados por $S(u) = \sin u$ y $T(u) = \cos u$. Defina de manera precisa todos los espacios usados para generar usar la regla de la cadena. 
    ...
    \item Considere el dominio $\Omega=(a,b)$ con $a<b$.
            \begin{itemize}
                ...
                \item\pts{1 extra} Para este problema, al calcular las derivadas numéricas en los bordes, tendrá problemas al usar diferencias finitas de orden 1 combinadas con diferencias centradas, lo que deteriorará la convergencia. Si bien esto no es un problema, puede elegir usar una fórmula de diferencias finitas unilaterales para obtener convergencia cuadrática en los extremos. Calculando las series de Taylor asociadas a $f(x+h)$ y $f(x+2h)$, demuestre que la aproximación correspondiente es
            $$ f'(x) = \frac{-3f(x) + 4 f(x+h) - f(x+2h)}{2h} + O(h^2). $$
            Use esta fórmula para mostrar que se recupera la convergencia cuadrática de las diferencias centradas.
            \end{itemize}
