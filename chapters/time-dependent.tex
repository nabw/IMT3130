...
\begin{enumerate}
    ...
    \begin{equation}
        \int_\Omega \ddot{u}v + \int_\Omega A u\cdot v = 0 \qquad \forall v\in V.
    \end{equation}
...
\begin{theorem}[A priori estimate, time-dependent parabolic problem]\label{thm:a-priori-continuous-time-dependent}
    ...
    \begin{proof}
        ...
        \end{equation*}
        which is well-defined since $\ten M$ is invertible due to the linear independence of the basis. Thus, there exists a unique solution $\dot{\vec u}$ to equation~\eqref{eq:weak-parabolic-IBVP-discrete}. 
...
\begin{equation}\label{eq:spacetime-estimate}
...
\example{
    One common example is the heat equation, stated as follows: Find $u$ in $L^2(0,T; H_0^1(\Omega))$ such that 
        $$\langle \dot u, v\rangle + a(u,v) = 0 \qquad\forall v \in H_0^1(\Omega),$$
where $a(u,v)\coloneqq (\grad u, \grad v)$. The previous developments show that we can approximate this problem using the k--th order FE space
    $$ V_h = \{ u_h \in C(\Omega): u_h|_T \in \mathbb{P}_k\}, $$
    using a backward Euler discretization in time, which yields the discrete system of finding $u^n_h$ in $V_h$
    $$ \frac{1}{\Delta t}(u_h^n, v_h) + a(u^n_h, v_h) = F(v_h)+ \frac{1}{\Delta t}(u_h^{n-1}, v_h) \qquad \forall v_h\in V_h,$$
    for given $u_h^0$. In fact, we obtain from~\eqref{eq:spacetime-estimate} the following convergence estimate: 
    $$ \|u_h^n - u(t^n) \|_0 \leq C(h^k + \Delta t) . $$
A common choice that stems from this estimate is to use $\Delta t = O(h^k)$.
}
