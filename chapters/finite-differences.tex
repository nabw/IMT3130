...
\begin{itemize}
    ...
    \begin{equation*}
        D^+f(x_i) \coloneqq \frac{f_{i+1} - f_i}{h_{i}}
    \end{equation*}
    \item Backward difference: 
    \begin{equation*}
        D^-f(x_i) \coloneqq \frac{f_{i} - f_{i-1}}{h_{i-1}}
    \end{equation*}
    \item Centered difference:
    \begin{equation*}
        Df(x_i) \coloneqq \frac{f_{i+1} - f_{i-1}}{h_{i} + h_{i-1}}
    \end{equation*}
\end{itemize}
...
\begin{theorem}[Lax equivalence theorem]\label{thm:lax-equivalence}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \end{tightalign*}
            \item Convergence $\implies$ stability: For this proof we will require the solution $u$ to be continuous, that the discrete operator $\mat A_h$ is invertible, and that the inverse operator $\mathcal A$ is bijective and bounded, i.e. $\| \mathcal A \| \leq M$. By taking inverses, we can write stability as 
                \begin{equation*}
                ...
                \begin{equation*}
                    \| \mat A_h^{-1} \vec f_h \|_{\infty,h} \leq Ch^k + \| \mathcal A^{-1} f \|_0 \leq C h^k + C_2 \| f \|_0<\infty,
                \end{equation*}
...
\begin{enumerate}
    \item \emph{Newton-Raphson}: we look for solutions of a linearized problem and iterate. Abstractly, we solve the equation $F(\vec x)=0$ by considering an initial point $\vec x^0$ and then using Taylor's theorem for linearizing $F(\vec x^{k+1})$ (unknown) around the previous iteration result $\vec x^k$ (known):
    \begin{equation*}
    ...
    \begin{tightalign*}
        ...
        &\leq C\|\vec u_h^{k} - \vec u_h^{k-1}\|_{\infty, h}.\tag{$\sin$ is $1$-Lipschitz}
    \end{tightalign*}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
with $c\in \R$. We note that a result such as $\|u^{n+1}\|\leq c\|u^n\|$ for $c\leq 1$ results in $\|u_h\|_{\infty, h} \leq \|u^0\|_{\infty, h}$. This is indeed a stability result, so it will be common to study equations in the form $u^{n+1} = g(u^n)$. Given that our theory works for linear operators, we will start by studying a simple first-order equation:
\begin{equation}
    \begin{aligned}
        u'+cu &= f\\
        u(0) &= u_0.
