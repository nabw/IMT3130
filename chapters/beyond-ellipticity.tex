...
Now that we have characterized injectivity and surjectivity of operators, we can now postulate a generalized version of the Lax-Milgram lemma. Our continuous problem is the same as in Section~\ref{sec:galerkin}, where we seek to find $u\in U$ such that $a(u,v)=f(v)$ for all $v\in V$. In a general setting, it is enough to consider $U$ a Banach space and $V$ a reflexive Banach space, but we state the theorem in the particular case where $U$ and $V$ are Hilbert spaces. 

\begin{theorem}(Generalized Lax-Milgram; Banach-Ne\v{c}as-Babu\v{s}ka)
\label{theorem:generalized-lax-milgram}
    Consider $U$, $V$ Banach spaces with $V$ reflexive and a bounded bilinear form $a:U\times V \to \mathbb{R}$. Then, there exists a unique $u\in U$ for each $F\in V'$ such that 
    \begin{equation}
    ...
    \begin{enumerate}
        ...
        \begin{equation}\label{eq:inf-sup-condition}
            \sup_{u\in U,u\neq 0} \frac{a(u,v)}{\| u \|_{U}} \ge \alpha \| v \|_{V} \quad \forall v\in V\quad \text{ (surjective) }
        \end{equation}
        \item 
        \begin{equation}\label{eq:inf-sup-injectivity}
            \sup_{v\in V} a(u,v) > 0 \quad \forall u\in U,\,\, u\neq 0 \quad \text{ (injective) }.
        \end{equation}
    \end{enumerate}
    Moreover, the following a priori estimate holds: 
    \begin{equation}
        \|u\|_{U} \leq \frac{1}{\alpha}\|f\|_{V'}.
    \end{equation}
\end{theorem}
...
\begin{equation}
    \inf_{v\in V}\sup_{u\in U,u\neq 0} \frac{a(u,v)}{\| u \|_{U}\|v\|_{V}} \ge \alpha.
\end{equation}

We note that injectivity can be equivalently stated as an inf-sup condition simply by inverting the arguments in the surjectivity part of the proof. In fact, consider $U_N\subset U$ and $v'_N\in V'_N\subset V$ discrete spaces, and define as $A:U \to V'$ the linear operator induced by the bilinear form $a$:
    $$ \langle Au, v\rangle_{V'\times V} \coloneqq a(u,v) \qquad \forall u\in U, v\in V. $$
Then, we readily obtain that 
\begin{equation*}
    \|A'v_N'\|_{U'} = \sup_{u\in U}\frac{\langle u, A'v_N'\rangle}{\|u\|_U} \geq \sup_{u_N\in U_N}\frac{\langle u_N, A'v_N'\rangle}{\|u_N\|_U},
\end{equation*}
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
These bilinear forms make sense when looking at the operator equation from $A$. Both problems are well-posed are both operators are surjective, which implies that 
\begin{equation*}
...
\begin{definition}[Compact operator]\label{def:compact-operator}
    A linear operator $K:V \to V$ is \emph{compact} if the image of a bounded sequence admits a converging subsequence.
\end{definition}
...
Now that we know some examples of saddle points problems, a natural question is to ask for conditions for the existence and uniqueness of solutions. Fortunately, this has already been done and it's known as the \emph{Ladyzhenskaya-Babu\v{s}ka-Brezzi Theory}, typically denoted as LBB theory.
...
\begin{itemize}
    ...
    \end{equation}
    where $\tilde \beta = 1/(C+1)$. This concludes the proof. We note that it was actually sufficient to show that for each $q$ there existed an element $\tilde{\vec u}$ in the desired space, as it proved the surjectivity of the operator $B$, but showing the complete inf-sup estimate was more instructive. 
\end{itemize}
