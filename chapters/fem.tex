...
\paragraph{\texorpdfstring{The $\mathbb{P}_1$ Lagrange finite element}{The P1 Lagrange finite element}}
...
\end{equation}
\paragraph{\texorpdfstring{The $\mathbb{P}_k$ Lagrange finite elements}{The Pk Lagrange finite elements}}
...
\paragraph{\texorpdfstring{The polynomial space $\mathbb{P}_k$}{The polynomial space Pk}} Let $x = (x_1, \dots, x_d)$ and let $\mathbb{P}_k$ be the space of polynomials in the variables $x_1, \dots, x_d$, with real coefficients and of global degree at most $k$,
\begin{equation}
...
\paragraph{\texorpdfstring{The polynomial space $\mathbb{Q}_k$}{The polynomial space Qk}} Let $\mathbb{Q}_k$ be the polynomial space in the variables $x_1, \dots, x_d$, with real coefficients and of degree at most $k$ in each variable. In dimension 1, $\mathbb{Q}_k = \mathbb{P}_k$; in dimension $d \ge 2$
\begin{equation}
...
\section{\texorpdfstring{Finite elements in $H(\dive)$ and $H(\curl)$}{Finite elements in H(div) and H(curl)}}\label{sec:finite-elements-Hdiv-Hcurl}
\paragraph{\texorpdfstring{$H(\dive)$:}{H(div):}} Here we consider the local polynomial in $\R^d$ given by
\begin{equation}
...
\paragraph{\texorpdfstring{$H(\curl)$}{H(curl)}} The procedure for this space is roughly similar to the previous one. For it, we define the space
\begin{equation}
...
\begin{theorem}[$H(\curl)$ interpolation] \phantom{.}
    ...
    \end{itemize}
\end{theorem}
\paragraph{\texorpdfstring{$H^1$}{H1}} From the lemma regarding conforming spaces, we can immediately see that the following is a conforming space in $H^1$: 
\begin{equation}
...
\begin{theorem}
    ...
    \end{equation*}
\end{theorem}
\paragraph{\texorpdfstring{$L^2$}{L2}} We can finally write the space 
\begin{equation}
