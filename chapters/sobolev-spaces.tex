...
\subsection{Distributions and weak derivatives}

Classical derivatives require functions to be pointwise differentiable – a condition too restrictive for many problems in PDEs. The language of distributions, introduced by L. Schwartz, provides a rigorous framework in which every locally integrable function is infinitely differentiable in a generalised sense. The key idea is to interpret a function not by its pointwise values, but by its action on a space of very smooth ``test functions'' through integration. Differentiation is then transferred to the test function via integration by parts, giving a definition that makes sense for a much larger class of objects.

\begin{definition}[Test functions]
Let $\Omega\subset\mathbb R^n$ be open. The space of test functions is
\[
\mathcal D(\Omega):=C_c^\infty(\Omega)=\{\phi\in C^\infty(\Omega):\operatorname{supp}\phi\text{ is compact}\}.
\]
We say that a sequence $\{\phi_k\}_{k\in\mathbb N}\subset\mathcal D(\Omega)$ converges to $\phi\in\mathcal D(\Omega)$ in $\mathcal D(\Omega)$ if
\begin{itemize}
\item there exists a compact set $K\subset\Omega$ such that $\operatorname{supp}\phi_k\subset K$ for all $k$, and
\item for every multi‑index $\alpha$, $\partial^\alpha\phi_k\to\partial^\alpha\phi$ uniformly on $K$.
\end{itemize}
This notion of convergence defines a topology on $\mathcal D(\Omega)$, but for our purposes it is enough to work with sequences and this characterisation of convergence.
\end{definition}

\begin{definition}[Distribution]
A distribution is a continuous linear functional on $\mathcal D(\Omega)$. The space of all distributions is denoted by $\mathcal D'(\Omega)$, thus implying that it is simply the topological dual.
\end{definition}

\example{
Every locally integrable function $f\in L^1_{\text{loc}}(\Omega)$ defines a distribution $T_f$ by
\[
\langle T_f,\phi\rangle:=\int_\Omega f(x)\phi(x)\,dx\qquad\forall\phi\in\mathcal D(\Omega).
\]
Linearity is obvious, and continuity follows from the dominated convergence theorem because the supports of the test functions are contained in a fixed compact set. Such distributions are called \emph{regular}. Conversely, not every distribution arises from a function; those that do not are called \emph{singular}.
}

\example{
    Fix a point $x_0\in\Omega$. The \emph{Dirac delta distribution} $\delta_{x_0}$ is defined by
\[
\langle\delta_{x_0},\phi\rangle:=\phi(x_0)\qquad\forall\phi\in\mathcal D(\Omega).
\]
It is clearly linear and continuous (point evaluation is continuous in the topology of $\mathcal D$). It is singular because it cannot be represented by a locally integrable function.
}

\subsubsection{\texorpdfstring{Topology of $\mathcal D'$}{Topology of D'}}
Defining the topology of the space of distributions is not a trivial affair. In fact, one could be tempted to use the topology induced by the dual norm 
$$ \| T \|_{\mathcal D'} = \sup_{x \in \mathcal D} \frac{|\langle T, x\rangle |}{\|x\|_{\mathcal{D}}} , $$
but the issue is that the space $\mathcal D$ is \emph{not metrizable} and thus the term $\|x\|_{\mathcal D}$ is not well-defined. You may find a proof of this in \cite{MR1157815}. For this reason, the correct topology is simply the weak$^*$ topology, meaning that a sequence $(T_n)_n$ in $\mathcal D'$ converges to $T$ in $\mathcal D'$ if $\langle T_n, \varphi\rangle \to \langle T, \varphi\rangle$ for all $\varphi$ in $\mathcal D$. 


\subsubsection{Distributional derivatives}
The great power of distributions lies in the fact that every distribution is differentiable (infinitely many times) in a weak sense. The definition mimics integration by parts.

\begin{definition}[Distributional derivative]
Let $T\in\mathcal D'(\Omega)$ and let $\alpha$ be a multi‑index. The distributional derivative $\partial^\alpha T$ is the distribution defined by
\[
\langle\partial^\alpha T,\phi\rangle:=(-1)^{|\alpha|}\langle T,\partial^\alpha\phi\rangle\qquad\forall\phi\in\mathcal D(\Omega).
\]
The right‑hand side is well defined because $\partial^\alpha\phi\in\mathcal D(\Omega)$, and the map $\phi\mapsto\langle T,\partial^\alpha\phi\rangle$ is continuous; hence $\partial^\alpha T$ is indeed a distribution.
\end{definition}

\example{
    \begin{itemize}
        \item If $f$ is a continuously differentiable function, then the distributional derivative of $T_f$ coincides with the distribution generated by the classical derivative $\partial^\alpha f$, because integration by parts gives
            \[
            \langle T_{\partial^\alpha f},\phi\rangle=\int_\Omega (\partial^\alpha f)\phi =(-1)^{|\alpha|}\int_\Omega f\,\partial^\alpha\phi =(-1)^{|\alpha|}\langle T_f,\partial^\alpha\phi\rangle.
            \]
        \item The derivative of the Dirac delta is
            \[
            \langle\delta'_{x_0},\phi\rangle = -\langle\delta_{x_0},\phi'\rangle = -\phi'(x_0).
            \]
        \item The Heaviside function $H(x)=1$ for $x>0$, $H(x)=0$ for $x<0$ (defined on $\mathbb R$) has distributional derivative $H'=\delta_0$:
            \[
...
\begin{remark}[Weak derivatives in Sobolev spaces]
When a distribution $T$ can be identified with a function $u\in L^1_{\text{loc}}(\Omega)$ and its distributional derivative $\partial^\alpha T$ can also be identified with a function $v\in L^1_{\text{loc}}(\Omega)$, we say that $v$ is the \emph{weak derivative} of $u$ and write $\partial^\alpha u=v$. This is exactly the notion that will be used to define Sobolev spaces in the next section.
\end{remark}

For vector‑valued distributions $\mathbf T=(T_1,\dots,T_n)$, we define the distributional divergence and curl componentwise in the obvious way, e.g.
\[
\langle\operatorname{div}\mathbf T,\phi\rangle := -\langle\mathbf T,\nabla\phi\rangle = -\sum_{i=1}^n\langle T_i,\partial_i\phi\rangle\qquad(\phi\in\mathcal D(\Omega)),
\]
and for $\mathbf T$ in $\mathbb R^3$,
\[
\langle\operatorname{curl}\mathbf T,\boldsymbol\psi\rangle := \langle\mathbf T,\operatorname{curl}\boldsymbol\psi\rangle\qquad(\boldsymbol\psi\in\mathcal D(\Omega)^3).
\]
...
\begin{theorem}[Trace theorem]\label{thm:trace-theorem}
    Let $\Omega$ be a bounded and Lipschitz domain. Then, considering $1/p < s \leq 1$, there exists a continuous \emph{and surjective} extension of $\gamma_0$ given by $\gamma_0: W^{s,p}(\Omega) \to W^{s-1/p, p}(\partial\Omega)$. The image of the trace operator, $W^{s-1/p, p}(\partial\Omega)$, is called the \emph{trace space} associated to $W^{s,p}(\Omega)$.
    \begin{proof}
...
\paragraph{The trace spaces} The trace space $H^{1/2}(\partial\Omega)$ has some nice properties, which we detail now. Its norm\footnote{Sobolev spaces of fractional order are a beast on their own. The rigorous definition can be given using either Fourier transforms or by using the Slobodeckij seminorm. Both are very cumbersome and seldom used.} is given by 
...
\end{equation}
More generally, this type of definition was originally used Sobolev spaces, and their distinction was not clear. On one hand, one defines the Sobolev space $W^{m,p}(\Omega)$ as the space of $L^p(\Omega)$ functions whose weak derivatives up to order $m$ are in $L^p(\Omega)$ as well. On the other hand, one can also define the space $H^{m,p}(\Omega)$ as
$$ H^{m,p}(\Omega) \coloneqq \overline{C^\infty(\Omega)\cap W^{m,p}(\Omega)}^{\|\cdot \|_{W^{m,p}(\Omega)}}, $$
    i.e. the closure of $C^\infty(\Omega)\cap W^{m,p}(\Omega)$ in the $W^{m,p}(\Omega)$ norm. It has been shown that both definitions coincide, in a result known as the Meyers-Serrin Theorem \cite{adams2003sobolev}\footnote{The original work is very short and a nice read. It is also notably unciteable. Here is the DOI: https://doi.org/10.1073/pnas.51.6.1055}.
...
\paragraph{\texorpdfstring{$H(\dive)$ and the normal trace}{H(div) and the normal trace}} In this space, we have a first simple density result: 
...
\paragraph{\texorpdfstring{$H(\curl)$ and the tangential traces}{H(curl) and the tangential traces}} In the proofs involving $H(\dive)$, we heavily used the fact that the $\grad$ and $\dive$ are the transpose of one another. This is not the case for the $\curl$ operator, so the proofs for this case are notoriously more complicated. Instead, it will suffice to state the relevant results:
\begin{itemize}
