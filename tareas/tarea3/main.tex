...
\begin{enumerate}
    ...
    \item\pts{4}  Considere $\Omega\subset \R^d$ Lipschitz y acotado, una función $\ten K:\Omega \to \R^{d\times d}$ en $L^\infty(\Omega,\R^{d\times d})$ tal que es simétrica, continua y definida positiva en casi todo punto ($\exists c, C>0: c|\vec x|^2 \leq \vec x^T\ten K\vec x \leq C|\vec x|^2$ c.t.p), una función $\vec b$ en $L^\infty(\Omega)$ tal que $\dive {\vec b}=0$, $\vec b\cdot \vec n\geq 0$ en $\Gamma_N$ y $\vec b\in H(\dive; \Omega)$, una función escalar positiva $a>0$ en $L^\infty(\Omega)$, y un elemento $f$ de $[H^1(\Omega)]'$. En este contexto, considere el problema de Advección-Difusión-Reacción (ADR)
            $$ 
            \begin{aligned}
